...
class OTA5tGymEnv(gym.Env):
    ...
    def _calc_cost(self, perf):
        ...
        if perf['Power'] > self.target_specs['Power_max']:
            ...
            penalty += (excess_power ** 2) * 1000.0 # V4: Extremely high penalty for power violations
            
        # Hard Constraint: Max Width (150 um)
        max_W = max([
            perf['devices']['M1']['W'],
            perf['devices']['M2']['W'],
            perf['devices']['M3']['W'],
        ...
        if max_W > 150e-6:
            excess_w = (max_W - 150e-6) / 1e-6 # in um
            penalty += (excess_w ** 2) * 1000.0 # V4: Massive penalty for huge transistors
