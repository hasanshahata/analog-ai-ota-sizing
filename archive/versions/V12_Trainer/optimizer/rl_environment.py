...
class OTA5tGymEnv(gym.Env):
    """
    V12 Goal-Conditioned Gymnasium environment for RL agents.
    Fixes from V11:
    1. Scaled reward (-cost/100) instead of clipped (-10,0) — full gradient preserved
    2. No early termination (V11 fix preserved)
    3. Smooth normalized cost function (V10 fix preserved)
    """
    ...
    def step(self, action):
        ...
        # V12 FIX: Scale reward instead of clipping. No information is lost.
        # cost=0 → reward=0, cost=10 → -0.1, cost=50 → -0.5, cost=500 → -5.0
        # The agent can now distinguish fine-grained cost differences and
        # learn target-conditional behavior (goal-conditioned policy).
        reward = -new_cost / 100.0
