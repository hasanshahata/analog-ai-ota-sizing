...
class OTA5tGymEnv(gym.Env):
    ...
    def _calc_cost(self, perf):
        """Dynamic cost based on current targets with HARD constraints."""
        if perf is None:
            return 1e9
        ...
        if perf['DC_Gain_dB'] < self.target_specs['Gain_min']:
            penalty += min(((self.target_specs['Gain_min'] - perf['DC_Gain_dB']) / 10.0) ** 2, 50.0)
            
        if perf['GBW'] < self.target_specs['GBW_min']:
            penalty += min(((self.target_specs['GBW_min'] - perf['GBW']) / 100e6) ** 2, 50.0)
            
        # Hard Constraint: Power
        if perf['Power'] > self.target_specs['Power_max']:
            excess_power = (perf['Power'] - self.target_specs['Power_max']) / 1e-6 # in uW
            penalty += 10.0 + min(excess_power * 0.1, 50.0) # V6: Normalized Cliff
            
        # Hard Constraint: NMOS Max Width (250 um)
        max_W_nmos = max([
            perf['devices']['M1']['W'],
            perf['devices']['M2']['W'],
            perf['devices']['M5']['W']
        ...
        if max_W_nmos > 250e-6:
            excess_w_nmos = (max_W_nmos - 250e-6) / 1e-6 # in um
            penalty += 10.0 + min(excess_w_nmos * 0.1, 50.0) # V6: Normalized Cliff
            
        # Hard Constraint: PMOS Max Width (750 um)
        max_W_pmos = max([
            perf['devices']['M3']['W'],
            perf['devices']['M4']['W']
        ])
        if max_W_pmos > 750e-6:
            excess_w_pmos = (max_W_pmos - 750e-6) / 1e-6 # in um
            penalty += 10.0 + min(excess_w_pmos * 0.1, 50.0) # V6: Normalized Cliff
            
        # PM minimum constraint (User requested 45 degrees minimum)
        if perf['PM'] < 45.0:
            penalty += min(((45.0 - perf['PM']) / 5.0) ** 2 * 10.0, 50.0)
            
        # Saturation constraints to keep physics valid
        if perf['min_sat_margin'] < 0.05:
            penalty += min(((0.05 - perf['min_sat_margin']) / 0.01) ** 2 * 10.0, 50.0)
            
        # Area Penalty: Area in m^2. Typically ~1e-12.
        # Multiply by 1e12 to bring to reasonable scale, add to cost.
        area_cost = perf['Area'] * 1e12 * 0.1 
        
        # Reward reducing power even if constraints are met
        base_cost = perf['Power'] * 1e6
        
        return base_cost + area_cost + (penalty * 1e4)
