...
class OTA5tGymEnv(gym.Env):
    ...
    def _calc_cost(self, perf):
        ...
        if perf['Power'] > self.target_specs['Power_max']:
            ...
            penalty += (excess_power ** 2) * 100.0 # V5: Relaxed penalty for power violations
            
        # Hard Constraint: NMOS Max Width (150 um)
        max_W_nmos = max([
            perf['devices']['M1']['W'],
            perf['devices']['M2']['W'],
            perf['devices']['M5']['W']
        ])
        if max_W_nmos > 150e-6:
            excess_w_nmos = (max_W_nmos - 150e-6) / 1e-6 # in um
            penalty += (excess_w_nmos ** 2) * 1000.0 # V5: NMOS penalty
            
        # Hard Constraint: PMOS Max Width (500 um)
        max_W_pmos = max([
            perf['devices']['M3']['W'],
            perf['devices']['M4']['W']
        ])
        if max_W_pmos > 500e-6:
            excess_w_pmos = (max_W_pmos - 500e-6) / 1e-6 # in um
            penalty += (excess_w_pmos ** 2) * 1000.0 # V5: PMOS penalty
            
        # PM minimum constraint (User requested 45 degrees minimum)
        if perf['PM'] < 45.0:
            penalty += ((45.0 - perf['PM']) / 5.0) ** 2 * 100.0 # Massive penalty for instability
            
        # Saturation constraints to keep physics valid
        if perf['min_sat_margin'] < 0.05:
            penalty += ((0.05 - perf['min_sat_margin']) / 0.01) ** 2 * 10.0
