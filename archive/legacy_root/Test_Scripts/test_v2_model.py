...
def main():
    ...
    model = PPO.load("universal_ppo_agent_65nm_v2.zip")
    ...
    for t in tests:
        ...
        # Override the environment's randomized targets with our test specs
        env.target_specs = specs
        
        # Reset manually to avoid resampling targets
        env.current_step = 0
        ...
        for _ in range(50):
            ...
            # Manually step so we don't trigger reset inside step if we don't want to
            env.current_step += 1
        ...
        print(f"Target Gain: {specs['Gain_min']}, Achieved: {final_perf['DC_Gain_dB']:.2f}")
        print(f"Target GBW: {specs['GBW_min']/1e6}M, Achieved: {final_perf['GBW']/1e6:.2f}M")
        print(f"Target CL: {specs['CL_pF']}p, Achieved: {specs['CL_pF']}p")
        print(f"Target Power: {specs['Power_max']*1e6}u, Achieved: {final_perf['Power']*1e6:.2f}u")
        
    # Build markdown table
    md = "# V2 Evaluation Results\n\n"
    ...
    with open('v2_evaluation_results.md', 'w') as f:
        f.write(md)
    print("\nSaved results to v2_evaluation_results.md")
