...
def test_size_passes_frozen_guard_policy(monkeypatch):
    captured: dict = {}
    rt = _ready_runtime(monkeypatch, captured)
    rt.size(CANON, seed=7)
    # the runtime itself does not alter the guard; sizing.py applies 1.25x
    assert captured["specs"]["GBW_min"] == 1e8
    assert rt.health()["policy_version"] == "tt-ideal-tail-gbw-v1"
...
def test_health_does_not_touch_engine():
    ...
    assert h["policy_version"] == "tt-ideal-tail-gbw-v1"
    rt.state = "failed"
    rt.detail = "required LUT or checkpoint files are missing (see server logs)"
